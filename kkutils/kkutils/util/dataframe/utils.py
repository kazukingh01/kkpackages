...
__all__ = [
    ...
    "is_integer_str_regex",
    "is_float_str_regex",
    "is_numeric_str_regex",
    "is_regex_pattern",
    "index_regex_pattern",
    "is_column_string",
    "is_columns_string",
    ...
    "to_pickles",
    "check_dataframe_diff",
    "sort_columns_based_first_character",
    "nanmap",
    "divide_index",
    "sort_each_columns",
    "create_sql_json_column",
]
...
def sort_each_columns(df: pd.DataFrame, **params) -> pd.DataFrame:
    ...
    for x in df.columns:
        dfret[x] = df[x].sort_values(**params).values.copy()
    return dfret

def create_sql_json_column(name_list: List[str], column_name: str, table_name: str=None) -> str:
    """
    postgresql の json type の column に対する sql の一部を作成する
    """
    sql = ""
    table_name = table_name + "."  if table_name is not None else ""
    for x in name_list:
        sqlwk = table_name + column_name + " ->> '" + x + "' as " + x + ","
        sql += sqlwk
    return sql[:-1]